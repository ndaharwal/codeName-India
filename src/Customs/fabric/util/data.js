...
function beautyStringify(value, offset, useDoubleQuotes) {
...
var utils = {
  ...
  beautyStringify (value,useDoubleQuotes){
    return beautyStringify(value,0,useDoubleQuotes)
  },
  _load_json (value, callback_success, callback_error){
    ...
    function _loadJson_data(data) {
      ...
      utils.recoursive(data,
        function (property, value, parent) {
          if (/^url\(.*\)$/.test(value)) {
            ...
            if (regex_data[1] === "./") {
              url = utils._getParentDirectoryUrl(filename) + url;
            }

            parent[property] = utils.resolvePath(url);
          }
        }
      );


      var loader = util.loader(0, function finalize() {
      ...
      utils.recoursive(data, function (property, value, parent) {
        ...
        if (/^\/\/.*$/.test(value) || /^@comment\(.*\)$/.test(value) || property === "@comment") {
          ...
          loader.push();
          utils._load_json(value, function (data) {
            var ext_data = util.deepExtend(data, parent);
            util.deepExtend(parent, ext_data);
          ...
          loader.push();
          utils._load_json(url, function (data) {
  ...
  _getParentDirectoryUrl: function (url) {
    return url.substr(0, url.lastIndexOf("/") + 1);
  },
  CACHED_JSON : {},
  loadJsonSync: function(src){
    var data;
    utils._load_json(src,function(_data){
      data = _data;
    });
    return data;
  },
  ...
  loadJson: function (value,resolve_cb,fail_cb) {
    return utils._load_json(value,function(data){
   ...
   */
  doubleQuote: function(jsonString){
  ...
  load: function (url, dataType = "json", callback_success, callback_error) {

    if(url.constructor === Object){
      let result = {};
      let promises = [];
      for(let key in url){
        promises.push(fabric.util.data.load(url[key]).then(data => {
          result[key] = data
        }));
      }
      return Promise.all(promises ).then(()=> {
        callback_success && callback_success(result);
        return result;
      }).catch(function (e) {
        callback_error && callback_error(e);
        return result;
      });
    }
    ...
    function parseData(data){
      // data = data.replace(/^\uFEFF/, '');
      let _parsed = utils.parseData(data, dataType);
      if(_parsed.status === "error"){
        if(callback_error){
          ...
          callback_success && callback_success(_parsed.data,_parsed.message);
        }
      }else{
        callback_success && callback_success(_parsed.data);
      }
      return _parsed.data;
    }
    ...
    if (isServer()) {
      try {
        if (url && url.indexOf('http') !== 0) {
          url = require("path").resolve(utils.resolvePath(url))
          return parseData(require('fs').readFileSync(url, 'utf8'));
        }
        else if (url) {
          let request = require("request");
          request.get(url, {encoding: "utf8"}, (err, res,data) => {
            parseData(data);
          });
        }
        ...
        if(callback_error){
          ...
          callback_success && callback_success(null,e);
        }
      }
    } else {
        ...
        httpRequest.onreadystatechange = function () {
          if (httpRequest.readyState === 4) {
            if (httpRequest.status === 200) {
              var data = parseData(httpRequest.responseText);
              resolve(data);
            } else {
              if(callback_error){
                callback_error(httpRequest);
                reject(httpRequest)
              }else{
                callback_success && callback_success(httpRequest.responseText,httpRequest);
